...
- Ошибки миграций/моделей: убедитесь, что все модели импортируются в `pereval_fastapi/app/db.py`.
 - При локальном запуске приложение по умолчанию автоматически создаёт отсутствующие таблицы в базе данных (удобно для разработки со SQLite). Чтобы отключить это поведение в окружении (например, в продакшне), установите переменную окружения `SKIP_CREATE_TABLES=1` перед запуском.
